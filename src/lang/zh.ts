...
export default {
	...
	SETTINGS_STRICT_FOLDERS_EXACT_DESC:
		"这些文件夹（及其子文件夹）中的所有笔记都将被强制进入严格只读模式。使用精确路径匹配。",
	...
	NOTICE_INVALID_STRICT_REGEX_PREFIX:
		"Enhanced Read Mode：无效的严格正则表达式模式：",
	NOTICE_INVALID_DEFAULT_REGEX_PREFIX:
		"Enhanced Read Mode：无效的默认正则表达式模式：",
	...
	COMMAND_TOGGLE_DEFAULT:
		"Enhanced Read Mode Control：切换当前笔记的默认只读状态",
	COMMAND_TOGGLE_STRICT:
		"Enhanced Read Mode Control：切换当前笔记的严格只读状态",
	...
	SETTINGS_PLUGIN_LANGUAGE_DESC:
		"选择此插件界面的显示语言。需要重启 Obsidian 或重新加载插件以应用更改。",
	SETTINGS_DIAGNOSTICS_TITLE: "配置诊断",
	CONFLICT_DIRECT_SAME_PATH_FILE_DEFAULT:
		"冲突：此文件（'{path}'）也存在于严格只读文件列表中。它将是严格的。",
	CONFLICT_DIRECT_SAME_PATH_FILE_STRICT:
		"冲突：此文件（'{path}'）也存在于默认只读文件列表中。根据优先级，它将是严格的。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_DEFAULT:
		"冲突：此文件夹（'{path}'）也存在于严格只读文件夹列表中。其中的所有笔记都将是严格的。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_STRICT:
		"冲突：此文件夹（'{path}'）也存在于默认只读文件夹列表中。根据优先级，其中的所有笔记都将是严格的。",
	CONFLICT_DEFAULT_FILE_IN_STRICT_FOLDER:
		"覆盖：此文件（'{path}'）位于默认列表中，但它位于严格文件夹“{conflictingPath}”内。它将是严格的。",
	CONFLICT_STRICT_FILE_IN_DEFAULT_FOLDER:
		"信息：此文件（'{path}'）位于严格列表中，并且位于默认文件夹“{conflictingPath}”内。文件特定的严格规则优先。",
	CONFLICT_DEFAULT_FILE_IN_DEFAULT_FOLDER_REDUNDANT:
		"冗余：此文件（'{path}'）已被“{conflictingPath}”的默认只读文件夹规则覆盖。",
	CONFLICT_STRICT_FILE_IN_STRICT_FOLDER_REDUNDANT:
		"冗余：此文件（'{path}'）已被“{conflictingPath}”的严格只读文件夹规则覆盖。",
};
