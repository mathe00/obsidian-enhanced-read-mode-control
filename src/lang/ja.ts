...
export default {
	...
	SETTINGS_PLUGIN_LANGUAGE_DESC:
		"このプラグインインターフェースの表示言語を選択します。変更を適用するにはObsidianの再起動またはプラグインの再読み込みが必要です。",
	SETTINGS_DIAGNOSTICS_TITLE: "構成診断",
	CONFLICT_DIRECT_SAME_PATH_FILE_DEFAULT:
		"競合: このファイル（'{path}'）は厳密読み取り専用ファイルリストにも存在します。厳密になります。",
	CONFLICT_DIRECT_SAME_PATH_FILE_STRICT:
		"競合: このファイル（'{path}'）はデフォルト読み取り専用ファイルリストにも存在します。優先度に従って厳密になります。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_DEFAULT:
		"競合: このフォルダー（'{path}'）は厳密読み取り専用フォルダーリストにも存在します。内部のすべてのノートは厳密になります。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_STRICT:
		"競合: このフォルダー（'{path}'）はデフォルト読み取り専用フォルダーリストにも存在します。内部のすべてのノートは優先度に従って厳密になります。",
	CONFLICT_DEFAULT_FILE_IN_STRICT_FOLDER:
		"上書き: このファイル（'{path}'）はデフォルトリストにありますが、厳密フォルダー '{conflictingPath}' 内にあります。厳密になります。",
	CONFLICT_STRICT_FILE_IN_DEFAULT_FOLDER:
		"情報: このファイル（'{path}'）は厳密リストにあり、デフォルトフォルダー '{conflictingPath}' 内にあります。ファイル固有の厳密ルールが優先されます。",
	CONFLICT_DEFAULT_FILE_IN_DEFAULT_FOLDER_REDUNDANT:
		"冗長: このファイル（'{path}'）は、'{conflictingPath}' のデフォルト読み取り専用フォルダー ルールによって既にカバーされています。",
	CONFLICT_STRICT_FILE_IN_STRICT_FOLDER_REDUNDANT:
		"冗長: このファイル（'{path}'）は、'{conflictingPath}' の厳密読み取り専用フォルダー ルールによって既にカバーされています。",
};
