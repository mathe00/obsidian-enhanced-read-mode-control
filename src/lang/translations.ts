...
export function loadTranslations(plugin: EnhancedReadModeControlPlugin): void {
	...
	// 2. If choice is 'auto' or invalid, use detection logic
		...
		plugin.logDebug(
			`Plugin language set to 'auto'. Obsidian (moment.locale()): ${momentLocale}`,
		);
	...
	if (targetLocale && translations[targetLocale]) {
		...
		plugin.logDebug(
			`Loaded translations for exact locale '${targetLocale}'`,
		);
		...
		if (translations[baseLocale]) {
			...
			plugin.logDebug(
				`Loaded translations for base locale '${baseLocale}' (from '${targetLocale}')`,
			);
		} else {
			...
			plugin.logDebug(
				`Locale '${targetLocale}' (or base '${baseLocale}') not found, falling back to 'en'.`,
			);
		}
	} else {
...
export function t(key: string, vars?: Record<string, string>): string {
	let text:
		...
		| undefined = activeTranslations; // Use activeTranslations
	...
	if (typeof text !== "string") {
		...
		let fallbackText:
			| string
			| Record<string, string | Record<string, string>>
			| undefined = en; // Default to English pack
		for (const part of keyParts) {
			if (
				fallbackText &&
				typeof fallbackText === "object" &&
				part in fallbackText
			) {
