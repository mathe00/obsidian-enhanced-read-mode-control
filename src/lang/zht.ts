...
export default {
	...
	SETTINGS_STRICT_FOLDERS_EXACT_DESC:
		"這些資料夾（及其子資料夾）中的所有筆記都將被強制以嚴格唯讀模式開啟。使用精確路徑匹配。",
	...
	SETTINGS_PLUGIN_LANGUAGE_DESC:
		"選擇此外掛程式介面的顯示語言。需要重新啟動 Obsidian 或重新載入外掛程式以套用變更。",
	SETTINGS_DIAGNOSTICS_TITLE: "設定診斷",
	CONFLICT_DIRECT_SAME_PATH_FILE_DEFAULT:
		"衝突：此檔案（'{path}'）也存在於嚴格唯讀檔案清單中。它將是嚴格的。",
	CONFLICT_DIRECT_SAME_PATH_FILE_STRICT:
		"衝突：此檔案（'{path}'）也存在於預設唯讀檔案清單中。根據優先順序，它將是嚴格的。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_DEFAULT:
		"衝突：此資料夾（'{path}'）也存在於嚴格唯讀資料夾清單中。其中的所有筆記都將是嚴格的。",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_STRICT:
		"衝突：此資料夾（'{path}'）也存在於預設唯讀資料夾清單中。根據優先順序，其中的所有筆記都將是嚴格的。",
	CONFLICT_DEFAULT_FILE_IN_STRICT_FOLDER:
		"覆寫：此檔案（'{path}'）位於預設清單中，但它位於嚴格資料夾「{conflictingPath}」內。它將是嚴格的。",
	CONFLICT_STRICT_FILE_IN_DEFAULT_FOLDER:
		"資訊：此檔案（'{path}'）位於嚴格清單中，並且位於預設資料夾「{conflictingPath}」內。檔案特定的嚴格規則優先。",
	CONFLICT_DEFAULT_FILE_IN_DEFAULT_FOLDER_REDUNDANT:
		"冗餘：此檔案（'{path}'）已被「{conflictingPath}」的預設唯讀資料夾規則覆蓋。",
	CONFLICT_STRICT_FILE_IN_STRICT_FOLDER_REDUNDANT:
		"冗餘：此檔案（'{path}'）已被「{conflictingPath}」的嚴格唯讀資料夾規則覆蓋。",
};
