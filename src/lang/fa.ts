...
export default {
	...
	SETTINGS_DEFAULT_FILES_EXACT_TITLE:
		"فایل‌های فقط خواندنی پیش‌فرض (مسیرهای دقیق)",
	SETTINGS_DEFAULT_FILES_EXACT_DESC:
		"فایل‌های لیست شده در اینجا به طور پیش‌فرض در حالت فقط خواندنی باز می‌شوند. از تطابق دقیق مسیر استفاده می‌کند.",
	SETTINGS_DEFAULT_FILES_EXACT_PLACEHOLDER:
		"مسیر فایل را وارد کنید (مثال: Notes/MyFile.md)",
	SETTINGS_DEFAULT_FOLDERS_EXACT_TITLE:
		"پوشه های فقط خواندنی پیش فرض (مسیرهای دقیق)",
	SETTINGS_DEFAULT_FOLDERS_EXACT_DESC:
		"تمام یادداشت های داخل این پوشه ها (و زیرپوشه ها) در حالت فقط خواندنی پیش فرض باز می شوند. از تطبیق دقیق مسیر استفاده می کند.",
	SETTINGS_DEFAULT_FOLDERS_EXACT_PLACEHOLDER:
		"مسیر پوشه را وارد کنید (مثلاً Projects/Active)",

	SETTINGS_STRICT_FILES_EXACT_TITLE:
		"فایل‌های فقط خواندنی سختگیرانه (مسیرهای دقیق)",
	SETTINGS_STRICT_FILES_EXACT_DESC:
		"فایل‌های لیست شده در اینجا مجبور به حالت فقط خواندنی می‌شوند. از تطابق دقیق مسیر استفاده می‌کند.",
	SETTINGS_STRICT_FILES_EXACT_PLACEHOLDER:
		"مسیر فایل را وارد کنید (مثال: Templates/Protected.md)",

	SETTINGS_STRICT_FOLDERS_EXACT_TITLE:
		"پوشه‌های فقط خواندنی سختگیرانه (مسیرهای دقیق)",
	SETTINGS_STRICT_FOLDERS_EXACT_DESC:
		"تمام یادداشت‌های داخل این پوشه‌ها (و زیرپوشه‌ها) مجبور به حالت فقط خواندنی سختگیرانه می‌شوند. از تطابق دقیق مسیر استفاده می‌کند.",
	SETTINGS_STRICT_FOLDERS_EXACT_PLACEHOLDER:
		"مسیر پوشه را وارد کنید (مثال: Archive/2023)",

	// Regex Path List Management
	SETTINGS_ENABLE_REGEX_TITLE: "فعال کردن تطابق Regex (بتا)",
	SETTINGS_ENABLE_REGEX_DESC_PART1:
		"فعال کردن تطابق مسیرهای فایل با عبارات منظم. این یک ویژگی بتا است. ",
	SETTINGS_ENABLE_REGEX_DESC_PART2: "با احتیاط استفاده کنید: ",
	SETTINGS_ENABLE_REGEX_DESC_PART3:
		"الگوهای regex نامعتبر می‌توانند باعث خطا یا رفتار غیرمنتظره شوند. تطابق Regex پس از تطابق دقیق پوشه/فایل بررسی می‌شود.",

	SETTINGS_DEFAULT_REGEX_TITLE: "فقط خواندنی پیش‌فرض (الگوهای Regex)",
	SETTINGS_DEFAULT_REGEX_DESC:
		"مسیرهای فایلی که با هر الگوی regex در اینجا مطابقت دارند، در حالت فقط خواندنی پیش‌فرض باز می‌شوند. یک الگوی regex جاوا اسکریپت در هر خط (بدون اسلش).",
	SETTINGS_DEFAULT_REGEX_PLACEHOLDER:
		"الگوی regex را وارد کنید (مثال: ^Journal/\\d{4}-\\d{2}-\\d{2}\\.md$)",

	SETTINGS_STRICT_REGEX_TITLE: "فقط خواندنی سختگیرانه (الگوهای Regex)",
	SETTINGS_STRICT_REGEX_DESC:
		"مسیرهای فایلی که با هر الگوی regex در اینجا مطابقت دارند، مجبور به حالت فقط خواندنی سختگیرانه می‌شوند. یک الگوی regex جاوا اسکریپت در هر خط (بدون اسلش).",
	SETTINGS_STRICT_REGEX_PLACEHOLDER:
		"الگوی regex را وارد کنید (مثال: ^Templates/.*)",
	...
	SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_ACTION_TEXT:
		" افزونه هر یادداشت معمولی را که در حالت فقط خواندنی یافت شود، هنگام باز شدن به حالت ویرایش برمی‌گرداند.",
	SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_BENEFIT_LABEL: "مزیت:",
	SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_BENEFIT_TEXT:
		' باگ "گیر افتادن در حالت فقط خواندنی" را برای پیمایش در همان تب برطرف می‌کند.',
	SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_DRAWBACK_LABEL: "عیب:",
	SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_DRAWBACK_TEXT:
		" انتخاب‌های دستی را لغو می‌کند. اگر یک یادداشت معمولی را به حالت فقط خواندنی تنظیم کنید، این افزونه هنگام باز کردن مجدد آن را به حالت ویرایش برمی‌گرداند.",
	...
	SETTINGS_NOTIFY_ON_MODE_CHANGE_DESC:
		"هنگامی که افزونه به طور فعال حالت نمایش یک یادداشت را هنگام باز شدن تغییر می‌دهد، یک اعلان کوتاه نشان دهید. این یک ویژگی بتا است.",
	SETTINGS_ENABLE_DEBUG_LOGGING_TITLE: "فعال کردن ثبت اشکال‌زدایی",
	SETTINGS_ENABLE_DEBUG_LOGGING_DESC:
		"نمایش گزارش‌های دقیق در کنسول توسعه‌دهنده. برای اعمال کامل تغییرات نیاز به راه‌اندازی مجدد آبسیدین یا بارگذاری مجدد افزونه است.",
	SETTINGS_ENABLE_CONFLICT_DETECTION_TITLE: "فعال کردن تشخیص تضاد",
	SETTINGS_ENABLE_CONFLICT_DETECTION_DESC:
		"تنظیمات را برای قوانین متضاد تجزیه و تحلیل کنید (مثلاً مسیر یکسان در لیست های پیش فرض و سختگیر). تضادها در لیست های زیر برجسته می شوند.",
	SETTINGS_NOTIFY_ON_CONFLICT_TITLE: "اطلاع رسانی در مورد تضادهای پیکربندی",
	SETTINGS_NOTIFY_ON_CONFLICT_DESC:
		"در صورت شناسایی تضادهای پیکربندی هنگام باز کردن یا تغییر تنظیمات، یک اعلان نشان داده شود.",
	...
	NOTICE_INVALID_STRICT_REGEX_PREFIX:
		"Enhanced Read Mode: الگوی regex سختگیرانه نامعتبر: ",
	NOTICE_INVALID_DEFAULT_REGEX_PREFIX:
		"Enhanced Read Mode: الگوی regex پیش‌فرض نامعتبر: ",
	...
	NOTICE_CHECK_SETTINGS_FOR_DETAILS:
		"برای جزئیات، تنظیمات افزونه را بررسی کنید.",
	NOTICE_CHECK_SETTINGS_FOR_DETAILS_INLINE:
		"برای جزئیات، لیست های زیر را بررسی کنید.",

	// Command Palette Command Names (from main.ts)
	COMMAND_TOGGLE_DEFAULT:
		"Enhanced Read Mode Control: تغییر حالت فقط خواندنی پیش‌فرض برای یادداشت فعلی",
	COMMAND_TOGGLE_STRICT:
		"Enhanced Read Mode Control: تغییر حالت فقط خواندنی سختگیرانه برای یادداشت فعلی",
	...
	SETTINGS_PLUGIN_LANGUAGE_DESC:
		"زبان نمایش رابط کاربری این افزونه را انتخاب کنید. برای اعمال تغییرات نیاز به راه‌اندازی مجدد آبسیدین یا بارگذاری مجدد افزونه است.",
	SETTINGS_DIAGNOSTICS_TITLE: "تشخیص پیکربندی",
	CONFLICT_DIRECT_SAME_PATH_FILE_DEFAULT:
		"تضاد: این فایل ('{path}') در لیست فایل های فقط خواندنی سختگیر نیز وجود دارد. سختگیر خواهد بود.",
	CONFLICT_DIRECT_SAME_PATH_FILE_STRICT:
		"تضاد: این فایل ('{path}') در لیست فایل های فقط خواندنی پیش فرض نیز وجود دارد. طبق اولویت سختگیر خواهد بود.",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_DEFAULT:
		"تضاد: این پوشه ('{path}') در لیست پوشه های فقط خواندنی سختگیر نیز وجود دارد. تمام یادداشت های داخل آن سختگیر خواهند بود.",
	CONFLICT_DIRECT_SAME_PATH_FOLDER_STRICT:
		"تضاد: این پوشه ('{path}') در لیست پوشه های فقط خواندنی پیش فرض نیز وجود دارد. طبق اولویت تمام یادداشت های داخل آن سختگیر خواهند بود.",
	CONFLICT_DEFAULT_FILE_IN_STRICT_FOLDER:
		"نادیده گرفتن: این فایل ('{path}') در یک لیست پیش فرض قرار دارد، اما داخل پوشه سختگیر '{conflictingPath}' است. سختگیر خواهد بود.",
	CONFLICT_STRICT_FILE_IN_DEFAULT_FOLDER:
		"اطلاعات: این فایل ('{path}') در یک لیست سختگیر و داخل پوشه پیش فرض '{conflictingPath}' قرار دارد. قانون سختگیر مخصوص فایل اولویت دارد.",
	CONFLICT_DEFAULT_FILE_IN_DEFAULT_FOLDER_REDUNDANT:
		"اضافی: این فایل ('{path}') قبلاً توسط قانون پوشه فقط خواندنی پیش فرض برای '{conflictingPath}' پوشش داده شده است.",
	CONFLICT_STRICT_FILE_IN_STRICT_FOLDER_REDUNDANT:
		"اضافی: این فایل ('{path}') قبلاً توسط قانون پوشه فقط خواندنی سختگیر برای '{conflictingPath}' پوشش داده شده است.",
};
