...
export function detectPathConflicts(
	...
	defaultReadOnlyFiles.forEach((df) => {
		...
		strictReadOnlyFolders.forEach((sfo) => {
			...
			if (
				normalizedDf.startsWith(normalizedSfo + "/") ||
				normalizedDf === normalizedSfo
			) {
				// Check if df is sfo or in sfo
				if (normalizedDf !== normalizedSfo) {
					// Only if it's a file *within* the folder
					conflicts.push({
						path: df,
						listType: "defaultFile",
	...
	strictReadOnlyFiles.forEach((sf) => {
		...
		defaultReadOnlyFolders.forEach((dfo) => {
			const normalizedDfo = norm(dfo);
			if (
				normalizedSf.startsWith(normalizedDfo + "/") ||
				normalizedSf === normalizedDfo
			) {
				if (normalizedSf !== normalizedDfo) {
					// Only if it's a file *within* the folder
					conflicts.push({
						path: sf,
						listType: "strictFile",
	...
	defaultReadOnlyFiles.forEach((df) => {
		...
		defaultReadOnlyFolders.forEach((dfo) => {
			const normalizedDfo = norm(dfo);
			if (
				normalizedDf.startsWith(normalizedDfo + "/") ||
				normalizedDf === normalizedDfo
			) {
				if (normalizedDf !== normalizedDfo) {
					// Only if it's a file *within* the folder
					conflicts.push({
						path: df,
						listType: "defaultFile",
	...
	strictReadOnlyFiles.forEach((sf) => {
		...
		strictReadOnlyFolders.forEach((sfo) => {
			const normalizedSfo = norm(sfo);
			if (
				normalizedSf.startsWith(normalizedSfo + "/") ||
				normalizedSf === normalizedSfo
			) {
				if (normalizedSf !== normalizedSfo) {
					// Only if it's a file *within* the folder
					conflicts.push({
						path: sf,
						listType: "strictFile",
...
export class ReadModeControlSettingTab extends PluginSettingTab {
	...
	private refreshConflictDisplay(): void {
		if (this.plugin.settings.conflictDetectionEnabled) {
			this.lastConflictCheckResult = detectPathConflicts(
				this.plugin.settings,
			);
			if (
				this.plugin.settings.notifyOnConflict &&
				this.lastConflictCheckResult.length > 0
			) {
				// Convert count to string for the translation function
				const summaryNotice = t("NOTICE_CONFLICTS_DETECTED_SUMMARY", {
					count: this.lastConflictCheckResult.length.toString(),
				});
				new Notice(
					summaryNotice +
						"\n" +
						t("NOTICE_CHECK_SETTINGS_FOR_DETAILS"),
					10000,
				); // Longer notice
			}
		} else {
	...
	private createListManagementUI(
		...
		const inputSetting = new Setting(containerEl).setClass(
			"ermc-path-input-setting",
		);
		...
		inputSetting.addButton((button: ButtonComponent) => {
			button
				.setButtonText(
					itemType === "regex"
						? t("SETTINGS_LIST_UI_ADD_REGEX_BUTTON")
						: t("SETTINGS_LIST_UI_ADD_PATH_BUTTON"),
				)
				.setIcon("plus")
				.onClick(async () => {
					...
					if (itemType === "file" || itemType === "folder") {
						...
						if (
							isValid &&
							!currentItems.includes(processedNewItem)
						) {
							currentItems.push(processedNewItem);
						} else if (!isValid) {
							this.plugin.logDebug(
								`Attempted to add invalid ${itemType} path: ${newItem} (normalized: ${processedNewItem})`,
							);
						} else {
							// Item already exists
							this.plugin.logDebug(
								`Path already exists: ${processedNewItem}`,
							);
						...
						try {
							...
							isValid = false;
							this.plugin.logDebug(
								`Attempted to add invalid regex: ${newItem}`,
							);
						}
					}
						...
						if (
							...
							new Notice(
								t(
									...
									{ path: processedNewItem },
								),
							);
						} else {
							new Notice(
								t("SETTINGS_LIST_UI_INVALID_PATH_OR_REGEX", {
									path: newItem,
									itemType: itemType,
								}),
							);
					...
					this.plugin.logDebug(
						`Added ${itemType}: ${processedNewItem}`,
					);
		...
		const pathListContainer = containerEl.createDiv(
			"ermc-path-list-container",
		);
	...
	private redisplayList(
		...
		if (items.length === 0) {
			...
			switch (itemType) {
				case "file":
					emptyMessage = t("SETTINGS_LIST_UI_EMPTY_FILES");
					break;
				case "folder":
					emptyMessage = t("SETTINGS_LIST_UI_EMPTY_FOLDERS");
					break;
				case "regex":
					emptyMessage = t("SETTINGS_LIST_UI_EMPTY_REGEX");
					break;
			}
			containerEl.createEl("p", {
				text: emptyMessage,
				cls: "setting-item-description ermc-empty-list-message",
			});
		}
		items.forEach((item, index) => {
			const itemConflicts = conflicts.filter(
				(c) => c.path === item && c.listType === listIdentifier,
			);
			...
			if (isConflict && this.plugin.settings.conflictDetectionEnabled) {
				...
				const tooltipMessages = itemConflicts
					.map((c) =>
						t(c.messageKey, {
							path: c.path,
							conflictingPath: c.conflictingPath || c.path,
						}),
					)
					.join("\n");
			...
			settingItem.addButton((button) =>
				button
					.setIcon("trash")
					.setTooltip(
						`${t("SETTINGS_LIST_UI_REMOVE_TOOLTIP_PREFIX")} ${item}`,
					)
					.onClick(async () => {
	...
	display(): void {
		...
		if (this.plugin.settings.conflictDetectionEnabled) {
			...
			if (this.lastConflictCheckResult.length > 0) {
				const diagnosticSection = containerEl.createDiv(
					"ermc-diagnostics-section",
				);
				diagnosticSection.createEl("h4", {
					text: t("SETTINGS_DIAGNOSTICS_TITLE"),
				});
				// Convert count to string for the translation function
				const summary = t("NOTICE_CONFLICTS_DETECTED_SUMMARY", {
					count: this.lastConflictCheckResult.length.toString(),
				});
				diagnosticSection.createEl("p", {
					text:
						summary +
						" " +
						t("NOTICE_CHECK_SETTINGS_FOR_DETAILS_INLINE"),
				});
			}
		} else {
			this.lastConflictCheckResult = [];
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				dropdown.addOption(
					"auto",
					languages["auto"] || "Automatic (Match Obsidian)",
				);
		...
		this.createListManagementUI(
			...
			async (newPaths) => {
				this.plugin.settings.defaultReadOnlyFolders = newPaths.map(
					(p) => normalizePath(p.replace(/^\/|\/$/g, "")),
				);
				await this.plugin.saveSettings();
			},
			"folder",
		...
		this.createListManagementUI(
			...
			async (newPaths) => {
				this.plugin.settings.strictReadOnlyFolders = newPaths.map((p) =>
			...
			.setDesc(
				createFragment((frag) => {
					...
					frag.createEl("strong", {
						text: t("SETTINGS_ENABLE_REGEX_DESC_PART2"),
					});
		...
		new Setting(containerEl)
			...
			.setDesc(
				createFragment((frag) => {
					frag.appendText(
						t("SETTINGS_FORCE_EDIT_UNMANAGED_DESC_INTRO"),
					);
					frag.createEl("br");
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_PROBLEM_LABEL",
						),
					});
					frag.appendText(
						t("SETTINGS_FORCE_EDIT_UNMANAGED_DESC_PROBLEM_TEXT"),
					);
					frag.createEl("strong", {
						text: t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION1_ACTION_LABEL",
						),
					});
					frag.appendText(
						t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION1_BENEFIT_LABEL",
						),
					});
					frag.appendText(
						t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION1_DRAWBACK_LABEL",
						),
					});
					frag.appendText(
						t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_ACTION_LABEL",
						),
					});
					frag.appendText(
						t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_BENEFIT_LABEL",
						),
					});
					frag.appendText(
						t(
					...
					frag.createEl("strong", {
						text: t(
							"SETTINGS_FORCE_EDIT_UNMANAGED_DESC_OPTION2_DRAWBACK_LABEL",
						),
					});
					frag.appendText(
						t(
		...
		containerEl.createEl("h3", {
			text: t("SETTINGS_SECTION_FEEDBACK_DEBUG"),
		});
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						...
						if (value) {
							this.plugin.logDebug(
								"Mode change notifications enabled.",
							);
							new Notice(t("NOTICE_NOTIFICATIONS_ENABLED"));
						} else {
							this.plugin.logDebug(
								"Mode change notifications disabled.",
							);
						}
					}),
			);
			...
			.addToggle((toggle: ToggleComponent) => {
				toggle
					...
					.onChange(async (value) => {
						...
						await this.plugin.saveSettings();
						if (value) {
							this.plugin.logDebug(`Debug logging enabled.`);
						}
					});
			});
	}
